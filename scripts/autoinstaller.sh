...
CONTAINER_NAME=delegates-supervisor
...
function configuring_nginx()
{
  ...
  lxc exec "${CONTAINER_NAME}" -- bash -c "cd '${WEBSITE_DIRECTORY}' && rm index*" &>/dev/null
...
function install()
{
  ...
  echo
  echo
  echo -e "${COLOR_PRINT_YELLOW}You can access a terminal inside the container using lxc exec ${CONTAINER_NAME} -- bash${END_COLOR_PRINT}"
}
