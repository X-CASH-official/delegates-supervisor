...
describe('HttpdataService', () => {
  ...
  it('Test GET request',inject([HttpTestingController, HttpdataService],(httpMock: HttpTestingController, HttpdataService: HttpdataService) => {
   HttpdataService.get_request(HttpdataService.GET_DELEGATES_STATISTICS).subscribe((event: HttpEvent<any>) => {
     switch (event.type) {
       case HttpEventType.Response:
          }
        });

        const mockReq = httpMock.match(HttpdataService.GET_DELEGATES_STATISTICS);
  ...
  it('Test POST request',inject([HttpTestingController, HttpdataService],(httpMock: HttpTestingController, HttpdataService: HttpdataService) => {
   HttpdataService.post_request(HttpdataService.GET_DELEGATES_STATISTICS,"DATA").subscribe((event: HttpEvent<any>) => {
     switch (event.type) {
       case HttpEventType.Response:
          }
        });
